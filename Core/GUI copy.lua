...
function GUI.CreateColumn(parent, width, height, point, relativeFrame, relativePoint, xOffset, yOffset, options)
    ...
    if options.showBackdrop ~= false then
        ...
        column:SetBackdropColor(options.bgColorR or 0.1, options.bgColorG or 0.1, options.bgColorB or 0.1, options.bgColorA or 1)
    end
    return column
end

-- Helper function to create text
function GUI.CreateText(parent, text, point, relativeFrame, relativePoint, xOffset, yOffset, fontSize, fontFlags, color)
    local textFrame = parent:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    textFrame:SetFont("Fonts\\FRIZQT__.TTF", fontSize or 12, fontFlags or "")
    textFrame:SetText(text)
    textFrame:SetPoint(point, relativeFrame, relativePoint, xOffset, yOffset)
...
function GUI.Enable(self)
    ...
    -- Title Text
    self.TitleText = self:CreateText("BNUI - Config", "CENTER", self.TitleContainer, "CENTER", 0, 0, 16, "OUTLINE", { r = 1, g = 1, b = 1, a = 1 })
    ...
    -- Columns
    self.CategoryList = self:CreateColumn(GuiWidth*0.2, GuiHeight-(Spacing*2), "TOPLEFT", self, "TOPLEFT", Spacing, -Spacing*1.5)
    self.SettingsArea = self:CreateColumn((GuiWidth-((GuiWidth*0.2)+(Spacing*2.5))), GuiHeight-(Spacing*2), "TOPLEFT", self.CategoryList, "TOPRIGHT", Spacing/2, 0)
